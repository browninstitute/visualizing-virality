...
function sketch(fp5) {
  ...
  fp5.draw = () => {
    if (user_on_network) {
      ...
      fp5.text(
        formatTime(timesecs),
        fp5.displayWidth / 30 / load_factor,
        (1.5 * ((fp5.displayHeight * 0.9) / 15)) / load_factor
      );
      ...
      if (first_eng + 4 <= timesecs && timesecs < first_eng + 9) {
        onboardingText =
          "That means some accounts won't see the tweet this time.";
        onboardingTextX = mainX - fp5.textWidth(onboardingText) / 2;
      }
      ...
      if (250 <= timesecs && timesecs < 630) {
        ...
        onboardingTextX = 0.1 * fp5.displayWidth;
        onboardingTextY = 0.75 * (fp5.displayHeight * 0.9);
      }

      if (630 <= timesecs && timesecs < 1260) {
        onboardingText =
          "It shows the proportion of people who didn't get to see the tweet this time.";
        onboardingTextX = 0.1 * fp5.displayWidth;
        onboardingTextY = 0.75 * (fp5.displayHeight * 0.9);
      }
      ...
      fp5.text(
        ...
        fp5.displayWidth / 30 + fp5.displayWidth / 60 + fp5.displayWidth / 30,
        (fp5.displayHeight * 0.9) / 2.5 +
          (fp5.displayHeight * 0.9) / 12 -
          upFactor
      );
      let rectWidth = fp5.textWidth(
        retweetSum + " retweets " + likeSum + " likes " + replySum + " replies"
      );
      fp5.textSize((fp5.displayHeight * 0.9) / 60 / load_factor);

      fp5.text(
        "Direct followers and unknown distance accounts that engaged with the tweet",
        ((3 / 2) * (fp5.displayWidth / 20)) / load_factor,
        (1.6 * fp5.displayHeight * 0.9) / 10 / load_factor + upFactor / 2
      );
      fp5.text(
        "2 degrees of separation from " + selection_user.name,
        ((3 / 2) * (fp5.displayWidth / 20)) / load_factor,
        (2 * fp5.displayHeight * 0.9) / 10 / load_factor + upFactor / 2
      );
      fp5.text(
        "3 or more degrees of separation from " + selection_user.name,
        ((3 / 2) * (fp5.displayWidth / 20)) / load_factor,
        (2.4 * fp5.displayHeight * 0.9) / 10 / load_factor + upFactor / 2
      );
      fp5.text(
        "Accounts who originally engaged with tweet, \n but would not under this level of demotion",
        ((3 / 2) * (fp5.displayWidth / 20)) / load_factor,
        (2.8 * fp5.displayHeight * 0.9) / 10 / load_factor + upFactor / 2
      );
      fp5.text(
        "Time",
        ((3 / 2) * (fp5.displayWidth / 11)) / load_factor,
        (9.3 * fp5.displayHeight * 0.9) / 10 / load_factor
      );

      fp5.text(
        "Number of Engagements",
        (1.3 * (fp5.displayWidth / 12)) / load_factor,
        ((6.4 * fp5.displayHeight * 0.9) / 10 - 60) / load_factor
      );
      fp5.text(
      ...
      fp5.rect(
        fp5.displayWidth / 30,
        (fp5.displayHeight * 0.9) / 2.5 - upFactor,
        rectWidth * 1.4,
        (fp5.displayHeight * 0.9) / 8,
        20
      );
      img.resize(fp5.displayWidth / 40, (fp5.displayHeight * 0.9) / 24);
      fp5.image(
        img,
        fp5.displayWidth / 30 + fp5.displayWidth / 60,
        (fp5.displayHeight * 0.9) / 2.5 +
          (fp5.displayHeight * 0.9) / 40 -
          upFactor
      );
  ...
  function makeKey() {
    ...
    fp5.rect(
      fp5.displayWidth / 20 / load_factor,
      (3 * fp5.displayHeight * 0.9) / 20 / load_factor + upFactor / 2,
      (0.75 * fp5.displayWidth) / 40
    );
    fp5.fill(102, 0, 153, 150);
    fp5.rect(
      fp5.displayWidth / 20 / load_factor,
      (3.7 * fp5.displayHeight * 0.9) / 20 / load_factor + upFactor / 2,
      (0.75 * fp5.displayWidth) / 40
    );
    fp5.fill(153, 0, 102, 150);
    fp5.rect(
      fp5.displayWidth / 20 / load_factor,
      (4.4 * fp5.displayHeight * 0.9) / 20 / load_factor + upFactor / 2,
      (0.75 * fp5.displayWidth) / 40
    );
    fp5.fill(200, 200, 200, 150);
    fp5.rect(
      fp5.displayWidth / 20 / load_factor,
      (5.1 * fp5.displayHeight * 0.9) / 20 / load_factor + upFactor / 2,
      (0.75 * fp5.displayWidth) / 40
    );
    ...
    fp5.text(
      "12h",
      2.1 * (fp5.displayWidth / 20),
      histogram_y + histogram_height + 15
    );
    fp5.text(
      "1d",
      3.5 * (fp5.displayWidth / 20),
      histogram_y + histogram_height + 15
    );
  ...
  Neuron.prototype.display = function () {
    ...
    let scaler =
      fp5.min(fp5.int(followerMap.get(this.name)) / 4000 + 10, 660) /
      load_factor;
